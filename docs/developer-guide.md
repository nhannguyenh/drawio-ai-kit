...
- **`src/installer.mjs`** — pure installer primitives (no I/O): `AGENT_REGISTRY` (one entry per supported agent — Claude Code, Claude Desktop, Codex, Gemini CLI, Antigravity CLI; each carries `id`, `kind`, `configPath`, optional `skillDir`, `present` probe); `mcpPayload`, `claudeCodeAddCommand`, `resolveSource`, `detectAgents`, `mergeJsonServers`, `mergeTomlServers`. Agents with `skillDir` get a SKILL.md symlink placed in that directory by `install.mjs` step 7b.
